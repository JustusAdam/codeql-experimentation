...
public:
    ...
    __attribute__((noinline)) void set_propval(const std::string &key, Value value, std::shared_ptr<Storelike> &store)
    {
...
public:
        ...
        bool validate_rights)
    {
        if (validate_signature)
        {
            ...
            catch (const std::exception &e)
            {
                throw std::runtime_error(
                    ...
                    "or serialization of the commit. Stringified commit: " +
                    stringified_commit);
            }
        }
        ...
        if (set)
        {
            ...
            {
                resource->set_propval(val.first, val.second, store);
            }
        }
